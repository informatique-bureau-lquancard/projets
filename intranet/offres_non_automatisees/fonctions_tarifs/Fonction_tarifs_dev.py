...
tab_formater_vin = [",", "DOUBLE MAGNUM", "MAGNUM", "JEROBOAM", "NDEG"]
...
tab_formater_prix = [" ", "<", ">", "+", "-", "\\U202F"]

def formaterQuantite(quantiteBis):

    if(quantiteBis is None):
        return "0"

    quantite : str = unidecode(quantiteBis).upper()
    quantite = supprimeChaineCaracteres(quantite , tab_formater_prix)
...
BO = ['73CL', '75CL', '77CL', '0.75', '75', '750', '750ML', 'BT.', 'BOUTEILLE']
QUART = ['20']
DE = ['37.5CL', '37,5', '0.375', '375', '375CL', '0.375CL', '1/2BT.', 'DEMI']
...
MG = ['150CL', '1.5', '1.5L', '150', 'MAG.', '1.75', 'MAGNUM']
MG_BIS = ['1500', '1.5']
MJ = ['225CL', '225', '2250']
...
def conditionnementNonDefaut(formatB : str, quantite : int, conditionnement : str, nom_vin : str):
    ...
    if(conditionnement in nom.tab_conditionnement):
        return conditionnement
...
def testValeur(listes: list, valeurTestee):
    
    indice : int = -1

    for liste in listes:

        indice = liste.index(valeurTestee)
