...
wb2 = Workbook()
dest_filename = 'sortie_ARIVOF.xlsx'
ws = wb2.active
ws.title = "ARIVOF ( ͡° ͜ʖ ͡°) "
...
for i  in range (1, row_count):
    ...
    if iVin[i].value is None :
        ...
        rec_cond = str(iCond[i].value).upper().replace('OWC','CBO').replace('0','').replace('OW','CBO').replace('PK','CC').replace('CTS','CC').replace('12CC','CC12').replace('6CC','CC6').replace(' ','').replace('CARDBOARD','CC')
        if 'CBO' in rec_cond :
